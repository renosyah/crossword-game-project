...
#		random.shuffle(temp_list) # randomize word list
		temp_list.sort_custom(Callable(MyCustomSorter, "sort_decending_arrLength")) # sort by length
#		temp_list.sort(key=lambda i: len(i.word), reverse=True) # sort by length
	...
	func compute_crossword(time_permitted = 1.00, spins=2):
		...
		var start_full = float(Time.get_unix_time_from_system())
#		var start_full = float(2)
#		var start_full = float(time.time())
		while (float(Time.get_unix_time_from_system()) - start_full) < time_permitted or count == 0: # only run for x seconds
...
#						except: pass
	...
	func sort_coordlist(coordlist, word): # give each coordinate a score, then sort
		...
		new_coordlist.sort_custom(Callable(MyCustomSorter, "sort_decending_bestScore")) # put the best scores first
...
#					outStr += '%s ' % string.ascii_lowercase[random.randint(0,len(string.ascii_lowercase)-1)]
	...
	func order_number_words(): # orders words and applies numbering system to them
		self.current_word_list.sort_custom(Callable(MyCustomSorter, "sort_ascending_numberWords")) # sort by length
